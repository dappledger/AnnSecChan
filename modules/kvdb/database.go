...
import (
	...
	"github.com/dappledger/AnnSecChan/modules/logger"
	"github.com/dappledger/AnnSecChan/modules/logger/glog"
	"github.com/dappledger/AnnSecChan/modules/metrics"
...
func (self *LDBDatabase) GetWithPrefix(prefix, seekey []byte, limit uint32) ([]*KVResult, error) {
	var (
		count   uint32
		results []*KVResult
	)
	...
	for iter.Next() {
		results = append(results, NewKVResult(iter.Key(), iter.Value()))
		count++
		if count >= limit {
			break
		}
...
func (dt *table) GetWithPrefix(prefix, lastKey []byte, limit uint32) ([]*KVResult, error) {
